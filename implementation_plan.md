...
### 실행 순서
1. **재현 테스트 베이스 마련** (출근, 스탠드업, 미팅룸 다중 접속)
2. **출퇴근 스토어의 다중 탭 동기화 사용자 가드 적용**
3. **Zustand 비동기 반응성 누수 수정** (대시보드 및 직원 뷰 셀렉터 전환)
4. **미팅룸 계약 정합성 수정 (Backend ↔ Frontend)** (`participantId` 필드 추가)
5. **회의 종료/퇴장 분기 및 카메라/마이크 제어 정상화**
6. **통합 컴파일 및 빌드 검증**
...
### 수동 검증 계획
1. **동일 브라우저 멀티 탭 격리 검증**: 탭 A(직원1), 탭 B(직원2) 각각 로그인 후 탭 A 출근 시 탭 B에 아무 영향이 없는지 검증.
2. **데일리 스탠드업 실시간 연동**: 직원이 데일리 스탠드업 작성 즉시 관리자 대시보드 피드에 데이터가 렌더링되는지 검증.
3. **화상회의 승인 및 입장 자동 제어**: 직원이 회의 참가 요청을 보냈을 때 호스트가 이를 정상 수신(승인)하고, 직원이 대기 상태에서 모달이 자동 팝업되어 입장되는지 테스트.
4. **장치 제어 및 회의 폭파 검증**: 카메라/마이크 토글 뱃지가 실시간으로 정확히 바뀌며, 호스트가 방을 나갔을 때 해당 방이 데이터베이스 및 대시보드 목록에서 완전히 삭제되는지 확인.
