  ...
  const getTeamMembersStatus = () => {
    return team.members.map((m) => {
      ...
      if (memberLogs.length > 0) {
        // 시간순 정렬 (최신순)
        const sorted = [...memberLogs].sort((a, b) => b.timestamp.localeCompare(a.timestamp));
        
        // 마지막 출퇴근 기록
        const lastIn = sorted.find((l) => l.type === 'IN');
        const lastOut = sorted.find((l) => l.type === 'OUT');
        
        lastCheckInTime = lastIn ? lastIn.timestamp.split(' ').slice(1).join(' ') : '';
        lastCheckOutTime = lastOut ? lastOut.timestamp.split(' ').slice(1).join(' ') : '';
      }
      ...
      if (m.id === user?.id) {
        ...
        lastCheckInTime = localCommuteLogs.find(l => l.employeeId === user.id && l.type === 'IN')?.timestamp.split(' ').slice(1).join(' ') || '';
      }
                ...
                {todayStandups.map((s) => (
                  <div key={s.id} className="p-5 rounded-2xl bg-slate-950 border border-slate-850 shadow-inner">
                    <div className="flex items-center justify-between border-b border-slate-900 pb-3 mb-3">
                      ...
                      <span className="text-[10px] text-slate-600 font-mono">{s.timestamp}</span>
                    </div>
