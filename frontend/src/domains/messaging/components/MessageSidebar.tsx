...
import { enterRoom, getRooms } from '../api';
...
export const MessageSidebar: React.FC = () => {
  ...
  // 3. 대화방이 이미 생성된 멤버와 그렇지 않은 멤버 구분
  const activeChatPartnerIds = new Set(rooms.map((r) => Number(r.otherMemberId)));
  ...
  const handleSelectMember = async (memberId: number) => {
    const existingRoom = rooms.find((r) => Number(r.otherMemberId) === Number(memberId));
    if (existingRoom) {
    ...
    try {
      ...
      const detail = await enterRoom(memberId);
